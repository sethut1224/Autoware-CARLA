...
public:
	...
	static PlannerHNS::Point rotatePoint(const PlannerHNS::Point &p, const PlannerHNS::Point &center, const PlannerHNS::Vector &axis,double theta)
	{
		double const c = cos(theta), s = sin(theta);
		double const C = 1.0 - c;
		PlannerHNS::Vector tmp = p - center;
		return center +	PlannerHNS::Vector(tmp[0] * (axis[0] * axis[0] * C + c) +
